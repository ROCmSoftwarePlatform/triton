...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    # if causal, it comes here 2 times and first goes to branch STAGE==1 and then into STAGE==2. If not causal, only comes here once and goes to the else branch (and goes over all the row blocks)
    if STAGE == 1:
        lo, hi = 0, start_m * BLOCK_M # until the current element (not including) in causal masking
    elif STAGE == 2:
        lo, hi = start_m * BLOCK_M, (start_m + 1) * BLOCK_M # to handle the current element
        lo = tl.multiple_of(lo, BLOCK_M)
    # causal = False
    else:
        lo, hi = 0, N_CTX
        # Go through the whole row of blocks
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        qk = tl.dot(q, k)
        if STAGE == 2: # does the causal masking
...
def _attn_fwd(Q, K, V, sm_scale, M, Out,  #
    ...
    # make this persisten = only assume that we have a limited set of pids that do not cover the 
    
    tl.static_assert(BLOCK_N <= HEAD_DIM)
    start_m = tl.program_id(0)
    off_hz = tl.program_id(1) # we have batch size * num heads as the max here
    off_z = off_hz // H # at which batch sample we are on
    off_h = off_hz % H # which head there
...
def _attn_fwd_persistent(Q, K, V, sm_scale, M, Out,  #
    ...
    while tile_id + NUM_SMS < num_tiles_total:
        ...
        # v_order: tl.constexpr = (1, 0) #(0, 1) if V.dtype.element_ty == tl.float8e5 else (1, 0)
        V_block_ptr = tl.make_block_ptr(
            base=V + qvk_offset,
            shape=(N_CTX, HEAD_DIM),
            strides=(stride_vk, stride_vn),
