...
def torch_moe(a, b, c, topk_ids, topk_weights, routed_weight, sorted_token_ids, expert_ids, num_tokens_post_padded,
    ...
    if int4:
        b = torch_int4_to_fp8_dequant(b, b_scale_int4)
...
def moe_gemm_kernel(a_ptr, b_ptr, c_ptr, a_scale_ptr, b_scale_ptr, b_scale_int4_ptr, topk_weights_ptr,
                    ...
                    top_k: tl.constexpr, compute_type: tl.constexpr, use_fp8_w8a8: tl.constexpr, even_Ks: tl.constexpr,
                    use_int4_w: tl.constexpr):
    """
...
def moe_gemm(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, a_scale: torch.Tensor, b_scale: torch.Tensor,
    ...
    use_fp8 = a_scale is not None and b_scale is not None
    use_int4 = b_scale_int4 is not None
    ...
    moe_gemm_kernel[grid](
        ...
        b_scale_int4.stride(0) if use_int4 is True else 0, b_scale_int4.stride(1) if use_int4 is True else 0, 0, 0,
        MUL_ROUTED_WEIGHT=topk_weights is not None, top_k=top_k, even_Ks=EVEN_K,
        compute_type=tl.bfloat16 if a.dtype == torch.bfloat16 else tl.float16, use_fp8_w8a8=use_fp8,
        use_int4_w=use_int4, **config)
    return c
