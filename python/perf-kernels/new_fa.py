...
def _attn_fwd(
    ...
    q_block_base = Q + qkv_offset + start_m * BLOCK_M * stride_qm
    q_ptrs = q_block_base + offs_qm[:, None] * stride_qm + offs_qk[None, :]

    k_block_base = K + qkv_offset
    k_ptrs = k_block_base + offs_n[None, :] * stride_kn + offs_qk[:, None]

    v_block_base = V + qkv_offset
    v_ptrs = v_block_base + offs_qk[None, :] * stride_vn + offs_n[:, None]
    ...
    acc = acc / l_i[:, None]
    # write back O
    o_block_base = Out + qkv_offset + start_m * BLOCK_M * stride_om
    offs_om = tl.arange(0, BLOCK_M)
    offs_on = tl.arange(0, BLOCK_N)
    o_ptrs = o_block_base + offs_om[:, None] * stride_om + offs_on[None, :]
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, sm_scale):
        ...
        _attn_fwd[grid](
            ...
            ## Enable slice_k_tile when dot-slicing is upstreamed
